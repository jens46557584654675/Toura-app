...
const memExp = (globalThis.__touraMemExp ??= new Map()); // key → expiry ms
const memDb = {
  async get(k){
    if(memExp.has(k) && memExp.get(k) < Date.now()){ mem.delete(k); memExp.delete(k); return null; }
    return mem.has(k) ? mem.get(k) : null;
  },
  async set(k, v, ttl){ mem.set(k, v); if(ttl) memExp.set(k, Date.now() + ttl * 1000); else memExp.delete(k); return 'OK'; },
  async del(k){ mem.delete(k); memExp.delete(k); return 1; },
};
...
// set(k, v, ttlSeconds?) — ttl auto-expires the key (reset tokens, rate counters).
export const db =
  restUrl && restToken ? {
    async get(k){ return (await getUpstash()).get(k); },
    async set(k, v, ttl){ return (await getUpstash()).set(k, v, ttl ? { ex: ttl } : undefined); },
    async del(k){ return (await getUpstash()).del(k); },
  } :
  redisUrl ? {
    async get(k){ const v = await (await getRedis()).get(k); return v == null ? null : JSON.parse(v); },
    async set(k, v, ttl){ return (await getRedis()).set(k, JSON.stringify(v), ttl ? { EX: ttl } : undefined); },
