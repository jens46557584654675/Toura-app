...
// Kantoor font — woff/woff2/ttf/otf, used both in the preview (@font-face) and
// burned into the export (Shotstack html asset @font-face).
export async function hostFont(dataUrl, ext){
  const m = String(dataUrl || '').match(/^data:[^;]*;base64,(.+)$/s);
  if(!m) throw new Error('Invalid font file');
  const buf = Buffer.from(m[1], 'base64');
  if(buf.length > 2 * 1024 * 1024) throw new Error('Font too large (max 2 MB)');
  const e = ['woff', 'woff2', 'ttf', 'otf'].includes(String(ext)) ? ext : 'woff2';
  if(!hasBlob()) return dataUrl; // local dev fallback
  const mime = { woff: 'font/woff', woff2: 'font/woff2', ttf: 'font/ttf', otf: 'font/otf' }[e];
  return putBlob(`branding/${crypto.randomUUID()}.${e}`, buf, mime);
}
