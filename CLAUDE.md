...
- **4 Final video** (view id `postprod`): three `.fstep` cards, each with a left circle **number badge** (`.fstep-num`, 34px, `--ink`/`--on-ink` so it inverts in dark), a big title + one grey sub-line, and the step's action on the right. **1 Upgrade your clips** (`renderFinalQuality` fills `#fq1Action` with the upgrade button / green "All clips in 720p" tick). **2 Video editing** — `#editPanel` is an exact scaled mirror of the popup editor's three rows (TEXT ⊕ / CLIPS with +Intro/thumbnails/+Outro / MUSIC); the whole panel opens the editor, and the ⊕ / "+ Add music" call `openEditor('text'|'music')` to jump straight to that part. **3 Export** (`renderExport` fills `#exSub`/`#exAction`/`#exBody` per state — idle button, render spinner, or the player + download).
